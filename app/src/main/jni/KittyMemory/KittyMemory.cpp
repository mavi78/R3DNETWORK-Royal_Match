...
ProcMap KittyMemory::getLibraryMap(const char *libraryName) {
    ProcMap retMap = {};
    retMap.startAddr = nullptr;
    retMap.endAddr = nullptr;

    struct CallbackData {
        const char* name;
        ProcMap* map;
    } data = { libraryName, &retMap };

    dl_iterate_phdr([](struct dl_phdr_info* info, size_t size, void* userData) -> int {
        auto* d = static_cast<CallbackData*>(userData);

        if (info->dlpi_name && strstr(info->dlpi_name, d->name)) {
            d->map->startAddr = (void*)info->dlpi_addr;

            // Kütüphane boyutunu hesapla
            uintptr_t endAddr = info->dlpi_addr;
            for (int i = 0; i < info->dlpi_phnum; i++) {
                if (info->dlpi_phdr[i].p_type == PT_LOAD) {
                    uintptr_t segEnd = info->dlpi_addr +
            ...
            d->map->endAddr = (void*)endAddr;
            d->map->length = endAddr - info->dlpi_addr;
            d->map->pathname = info->dlpi_name;
            d->map->perms = "r-xp";
            d->map->offset = 0;
            d->map->inode = 0;
            d->map->dev = "";
